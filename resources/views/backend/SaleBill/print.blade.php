...
<head>
    <meta charset="UTF-8">
    <title>Sale Challan #{{ $record->voucher_no }}</title>
...
<table class="no-border">
    <tr>
        <td><b>Date:</b> {{ \Carbon\Carbon::parse($record->bill_date)->format('d-m-Y') }}</td>
        <td><b>Challan No:</b> {{ $record->voucher_no }}</td>
    </tr>
    <tr>
        <td colspan="8" style="font-weight:700;">
            To,<br>
            {{ optional($record->party)->name }}<br>

            @if(optional($record->party)->address)
                {{ optional($record->party)->address }}<br>
            @endif

            @if(optional($record->party)->mobile)
                {{ optional($record->party)->mobile }}<br>
            @endif

            @if(optional($record->party)->gstin)
                GST No.:- {{ optional($record->party)->gstin }}
            @endif
        </td>
    </tr>
</table>
...
@php
    ...
    $sgstTotal = 0;
    $igstTotal = 0;
    $isSameState = optional($record->party)->state_id == optional($company)->state_id;
@endphp
...
<table>
    ...
    <tbody>
    @foreach ($record->saleBillItem as $k => $item)
        @php
    ...
    <tr>
        ...
        <td class="text-right"><b>{{ number_format($subTotal + $cgstTotal + $sgstTotal + $igstTotal, 2) }}</b></td>
    </tr>
    </tbody>
</table>
