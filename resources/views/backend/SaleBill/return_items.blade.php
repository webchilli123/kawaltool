        ...
        <div class="col-xs-12">
            ...
            <table id="item_table" class="table table-striped table-bordered order-column">
                ...
                <tbody>
                    ...
                    $counter = 0;
                    ?>
                    @foreach($saleBill->saleBillItem as $k => $saleBillItem)
                    <?php
                    $counter++;
                    ?>
                    <?php $id = $saleBillItem->id; ?>
                    <tr>
                        <td class="text-center">
                            <input type="hidden" name="sale_items[{{ $id }}][id]" value="{{ $saleBillItem['id'] }}" />
                            <input type="hidden" name="sale_items[{{ $id }}][item_id]" value="{{ $saleBillItem['item_id'] }}" />
                            {{ $counter }}
                        </td>
                        <td>
                            {{ $saleBillItem->item->display_name }}
                        </td>
                        <td>{{ $saleBillItem->item->unit->display_name }}</td>
                        <td>
                            <span class="qty">{{ $saleBillItem->qty }}</span>
                        </td>
                        <td>
                            <span class="moved_qty">{{ $saleBillItem->moved_qty }}</span>
                        </td>
                        <td>
                            <?php 
                            $net_qty = $saleBillItem->qty - $saleBillItem->moved_qty; 
                            $validate_cls = strtoupper($saleBillItem->item->display_name) == "PC" ? "validate-int" : "validate-float";
                            $value = $saleBillItem->return_qty;
                            ?>
                            <x-Inputs.text-field name="sale_items[{{ $id }}][return_qty]" label=""
                                :value="$value"
                        ...
                        <td>
                            <span class="rate">{{ $saleBillItem->rate }}</span>
                        </td>
                        <td>
                            <span class="igst_per">{{ $saleBillItem->igst_per }}</span>%

                            <br/>
                            <span class="return_igst">0</span>
                            <input type="hidden" name="sale_items[{{ $id }}][return_igst]" value="0" class="return_igst" />
                        </td>
                        <td>
                            <span class="sgst_per">{{ $saleBillItem->sgst_per }}</span>%

                            <br/>
                            <span class="return_sgst">0</span>
                            <input type="hidden" name="sale_items[{{ $id }}][return_sgst]" value="0" class="return_sgst" />
                        </td>
                        <td>
                            <span class="cgst_per">{{ $saleBillItem->cgst_per }}</span>%

                            <br/>
                            <span class="return_cgst">0</span>
                            <input type="hidden" name="sale_items[{{ $id }}][return_cgst]" value="0" class="return_cgst" />
                        </td>
                        <td>
                            <span class="return_amount"></span>
                            <input type="hidden" name="sale_items[{{ $id }}][return_amount]" value="0" class="return_amount" />
                        </td>
                    </tr>
                    @endforeach
                </tbody>
...
<script type="text/javascript">
    $(document).ready(function(){
        function cal_total_amounts() {
            ...
            var payable_amount = total_amount + total_igst + total_sgst + total_cgst - other_deduction;
